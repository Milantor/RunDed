...
public class Player : MonoBehaviour, IControllable
{
    ...
    private Vector2? _fixedVelocity = null; // Фиксация скорости

    private Rigidbody2D _rb;
    private Visual _animator;

    private void Start()
    {
        _rb = GetComponent<Rigidbody2D>();
        _animator = GetComponent<Visual>();

        _animator.AutoWalk = true;
    }
    ...
    IEnumerator dash()
    {
        _fixedVelocity = Constants.dashSpeed * _rb.velocity.normalized;
    ...
    public void Jump()
    {
        ...
        _rb.AddForce((_rb.velocity.normalized + new Vector2(0, 4)).normalized * Constants.jumpPower);
    }

    public void Move(float value)
    {
        if (value > 0)
            _rb.AddForce(new Vector2(Mathf.Clamp(Constants.maxWalkSpeed - _rb.velocityX, 0, Constants.maxWalkAcceleration), 0));
        else
            _rb.AddForce(new Vector2(-Mathf.Clamp(Constants.maxWalkSpeed + _rb.velocityX, 0, Constants.maxWalkAcceleration), 0));
    }
